<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      ...
      <NuxtLink to="/super-admin/testimonials/create" class="px-4 py-2 bg-brand-600 text-white rounded-lg font-bold text-sm hover:bg-brand-700 transition flex items-center shadow-lg shadow-brand-500/20">
        + Tambah Testimoni
      </NuxtLink>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="item in testimonials" :key="item.id" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col relative group">
        ...
        <NuxtLink :to="`/super-admin/testimonials/${item.id}`" class="mt-4 w-full block text-center py-2 bg-gray-50 text-gray-600 font-bold text-xs rounded hover:bg-gray-100">Edit</NuxtLink>
      </div>
    </div>
  </div>
</template>
