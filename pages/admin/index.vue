<template>
  <div>
    <h1 class="text-3xl font-bold text-gray-900 mb-2">Dashboard {{ unitName }}</h1>
    <p class="text-gray-500 mb-8" v-if="unitName !== 'Kesantrian'">Kelola pendaftaran santri baru untuk unit {{ unitName }}.</p>
    <p class="text-gray-500 mb-8" v-else>Kelola data kedisiplinan dan perizinan santri.</p>
    ...
    <!-- Registration Table (Show for Pendaftaran Admins) -->
    <div v-if="unitName !== 'Kesantrian'" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
        <h2 class="font-bold text-gray-900">Data Pendaftar Terbaru</h2>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="reg in registrations" :key="reg.id" class="hover:bg-gray-50 transition-colors">
            <td class="px-6 py-4 whitespace-nowrap">
              ...
              <div class="text-xs text-gray-400">NISN: {{ reg.nisn }}</div>
            </td>
            <td class="px-6 py-4 whitespace-nowrap">
    ...
    <!-- Kesantrian Table (Show for Admin Santri) -->
    <div v-if="unitName === 'Kesantrian'" class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
...
const unitName = computed(() => {
  if (auth.user?.role === 'admin:paud') return 'PAUD'
  if (auth.user?.role === 'admin:sd') return 'SDQTA'
  if (auth.user?.role === 'admin:pondok') return 'Pondok Pesantren'
  if (auth.user?.role === 'admin:santri') return 'Kesantrian'
  if (auth.user?.role === 'super') return 'Super Admin View'
  return '-'
})

// Mock Data - In real app, fetch based on unitName/role
const stats = ref({
  total: 45,
  verified: 32,
  pending: 13
...
onMounted(() => {
  // Simulate fetching data for specific unit
  if (unitName.value === 'PAUD') {
    registrations.value = [
       { id: 4, name: 'Fatimah Kecil', nisn: '-', parentName: 'Budi', phone: '081xxx', date: '2026-01-21', status: 'pending' }
    ]
    stats.value = { total: 12, verified: 5, pending: 7 }
  } else if (unitName.value === 'Kesantrian') {
     stats.value = { total: 500, verified: 480, pending: 20 } // Total Santri, etc
  }
})
</script>
