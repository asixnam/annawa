<template>
  <header class="fixed top-0 left-0 right-0 z-50 bg-white/70 dark:bg-gray-950/70 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-800/50 py-4 transition-colors duration-300">
    <div class="container mx-auto px-6">
      <div class="flex justify-between items-center">
        <!-- Logo -->
        <NuxtLink to="/" class="flex items-center">
          <img src="/annawa.png" alt="Logo" class="w-12 h-12 object-contain dark:brightness-0 dark:invert" />
          <div class="ml-3 hidden lg:block">
            <span class="block text-sm font-extrabold text-main leading-tight uppercase font-heading">Khozinatul Ulum</span>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="hidden lg:relative lg:block" @mouseenter="isCtaOpen = true" @mouseleave="isCtaOpen = false">
            <button class="px-7 py-3 bg-brand-500 text-black font-extrabold text-xs tracking-widest uppercase rounded shadow-sm hover:bg-white hover:text-black dark:hover:bg-white dark:hover:text-black transition-all duration-300 flex items-center gap-2">
              Daftar
          ...
          <!-- Mobile Toggle -->
          <button class="md:hidden p-2 rounded-md hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors" @click="isOpen = !isOpen">
    ...
    <div v-if="isOpen" class="md:hidden mt-4">
      <div class="bg-white/90 dark:bg-gray-950/90 backdrop-blur-md rounded-2xl shadow-lg px-6 py-4 space-y-1">
        ...
        <div>
          <button @click="isAboutMobileOpen = !isAboutMobileOpen" class="w-full text-left py-2 hover:text-primary flex items-center justify-between transition-colors uppercase font-bold text-sm">
            About
        ...
        <div>
          <button @click="isCtaMobileOpen = !isCtaMobileOpen" class="w-full text-left py-2 text-secondary font-bold flex items-center justify-between transition-colors uppercase text-sm">
            Pendaftaran
