      ...
      <div v-for="item in testimonials" :key="item.id" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col relative group">
        ...
        <div class="flex items-center gap-4 mb-4">
          <div class="w-12 h-12 rounded-full bg-gray-200 overflow-hidden flex-shrink-0">
            <img :src="item.avatar_url" class="w-full h-full object-cover">
          </div>
          <div>
        ...
        <p class="text-gray-600 text-sm italic mb-4 flex-grow">"{{ item.content }}"</p>
        <p class="text-xs text-gray-400 font-medium">{{ item.role }}</p>
