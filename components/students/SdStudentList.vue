  ...
  <div class="space-y-6">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      <div class="w-full lg:w-auto">
        <h1 class="text-2xl font-black text-main uppercase tracking-tight">Data Murid SDQTA</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1 font-medium">Manajemen data murid SDQTA An-Nawa.</p>
      </div>
        ...
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center">
            <div class="h-10 w-10 rounded-2xl bg-brand-100 dark:bg-brand-900/40 flex items-center justify-center text-brand-600 dark:text-brand-400 font-black mr-3 uppercase text-xs">
              {{ (item.namaLengkap || item.name)[0] }}
            </div>
            <div>
              <div class="text-sm font-black text-main uppercase leading-tight">{{ item.namaLengkap || item.name }}</div>
              <div class="text-[10px] text-gray-500 dark:text-gray-400 font-bold tracking-wider">{{ item.nik || '-' }}</div>
            </div>
          </div>
          <span class="px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest" :class="(item.jenisKelamin || item.jk) === 'laki-laki' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400'">
            {{ (item.jenisKelamin || item.jk) === 'laki-laki' ? 'L' : 'P' }}
          </span>
        </div>
    ...
    >
      <template #namaLengkap="{ item }">
        <div class="flex items-center">
          ...
          <div>
            <div class="text-sm font-bold text-gray-900 dark:text-white">{{ item.namaLengkap || item.name }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ item.noHp }}</div>
          </div>
        </div>
      </template>
      <template #nisn="{ item }">
        <div class="text-sm text-gray-900 dark:text-white font-bold">{{ item.nik || '-' }}</div>
        <div class="text-[10px] text-gray-400 dark:text-gray-500 font-medium">NISN: {{ item.nisn || '-' }}</div>
      </template>
      <template #jenisKelamin="{ item }">
        <span class="px-2.5 py-1 rounded-lg text-xs font-bold capitalize" :class="(item.jenisKelamin || item.jk) === 'laki-laki' ? 'bg-primary/10 text-primary' : 'bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-400'">
          {{ (item.jenisKelamin || item.jk) === 'laki-laki' ? 'L' : 'P' }}
        </span>
      </template>
...
const headers = [
  { key: 'namaLengkap', label: 'Murid' },
  { key: 'nisn', label: 'NIK / NISN' },
