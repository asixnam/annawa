<template>
  <div class="space-y-6">
    ...
    >
      ...
      <template #nisn="{ item }">
        <span class="text-sm text-gray-600 font-medium">{{ item.nisn || '-' }}</span>
      </template>
      <template #jk="{ item }">
        <span class="px-2.5 py-1 rounded-lg text-xs font-bold capitalize" :class="item.jk === 'putra' ? 'bg-blue-50 text-blue-700' : 'bg-pink-50 text-pink-700'">
          {{ item.jk }}
        </span>
      </template>
