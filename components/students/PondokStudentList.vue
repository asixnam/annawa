  ...
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        ...
        <p class="text-gray-500 text-sm mt-1">Manajemen data santri Pondok Pesantren Khozinatul Ulum.</p>
      </div>
    ...
    >
      <template #name="{ item }">
        <div class="flex items-center">
          <div class="h-10 w-10 rounded-xl bg-green-100 flex items-center justify-center text-green-600 font-bold mr-3 uppercase text-sm">
            {{ item.name[0] }}
          </div>
      ...
      <template #room="{ item }">
        <span class="text-sm text-gray-500 font-bold">{{ item.room }}</span>
      </template>
    </StudentTable>
  </div>
</template>
