  ...
  <div class="space-y-6">
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
      <div class="w-full lg:w-auto">
        <h1 class="text-2xl font-black text-main uppercase tracking-tight">Data Murid PAUD</h1>
        <p class="text-gray-500 dark:text-gray-400 text-sm mt-1 font-medium">Manajemen data murid PAUD Islam Terpadu An-Nawa.</p>
      </div>
    ...
    <div class="grid grid-cols-1 gap-4 md:hidden">
      <div v-for="item in filteredStudents" :key="item.id" class="bg-card p-5 rounded-3xl border border-gray-100 dark:border-brand-400/20 shadow-sm transition-all active:scale-[0.98]">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="px-2 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest" :class="item.jk === 'laki-laki' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'bg-pink-50 dark:bg-pink-900/20 text-pink-600 dark:text-pink-400'">
            {{ item.jk }}
          </span>
        </div>
      ...
      <div v-if="filteredStudents.length === 0" class="py-12 text-center bg-card rounded-3xl border border-dashed border-gray-200 dark:border-gray-800">
    ...
    >
      ...
      <template #jk="{ item }">
        <span class="px-2.5 py-1 rounded-lg text-xs font-bold capitalize" :class="item.jk === 'laki-laki' ? 'bg-primary/10 text-primary' : 'bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-400'">
          {{ item.jk }}
        </span>
      </template>
