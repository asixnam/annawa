  ...
  <div class="min-h-screen bg-gray-50 flex">
    ...
    <aside class="w-64 bg-white border-r border-gray-200 hidden md:flex flex-col fixed inset-y-0 z-50">
      <div class="h-16 flex items-center px-6 border-b border-gray-200">
        <NuxtLink to="/" class="flex items-center gap-2">
          ...
          <span class="font-bold text-gray-800 tracking-tight">An-Nawa Admin</span>
        </NuxtLink>
      </div>
        ...
        <nav class="space-y-1 px-3">
          ...
          <div v-if="auth.hasRole('admin:*') || auth.hasRole('super')">
            <p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 mt-6">Administrasi</p>
             <NuxtLink to="/admin" class="flex items-center px-3 py-2 text-sm font-medium rounded-lg text-gray-700 hover:bg-brand-50 hover:text-brand-600 group transition-all" active-class="bg-brand-50 text-brand-600">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-gray-400 group-hover:text-brand-500 group-[.router-link-active]:text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="auth.hasRole('author') || auth.hasRole('super')">
            <p class="px-3 text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 mt-6">Author</p>
             <NuxtLink to="/author" class="flex items-center px-3 py-2 text-sm font-medium rounded-lg text-gray-700 hover:bg-brand-50 hover:text-brand-600 group transition-all" active-class="bg-brand-50 text-brand-600">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3 text-gray-400 group-hover:text-brand-500 group-[.router-link-active]:text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <main class="flex-1 md:pl-64 flex flex-col min-h-screen">
      ...
      <header class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-4 md:hidden">
        <NuxtLink to="/" class="font-bold text-gray-800">An-Nawa Admin</NuxtLink>
      ...
      <div class="p-6 lg:p-10 flex-1">
       ...
       <footer class="bg-white border-t border-gray-200 py-4 px-6 text-center md:text-left">
        <p class="text-sm text-gray-500">&copy; {{ new Date().getFullYear() }} Pondok Pesantren Khozinatul Ulum An-Nawa. All rights reserved.</p>
      </footer>
    </main>
  </div>
</template>
