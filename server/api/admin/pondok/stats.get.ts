...
export default defineEventHandler(async (event) => {
    try {
        ...
        const { rows: _res_reg } = await pool.query(
            'SELECT COUNT(*) as total FROM students WHERE unit = $1', ['SANTRI']
        )
        const totalRegistrations = Number(_res_reg[0]?.total || 0)

        const { rows: _res_news } = await pool.query('SELECT COUNT(*) as total FROM news')
        const totalNews = Number(_res_news[0]?.total || 0)

        const { rows: _res_gallery } = await pool.query('SELECT COUNT(*) as total FROM gallery')
